...
export default () => {
  ...
  const deleteRecord=async (id)=>{
    setShowModal(false);
    const {data,status} =await repository.deletePtypefinishing(id).then(x=>x).then(x=>x);
    try {
      if (status != 200) {
        setOpen(true)
        ...
        const {data,status}  =await repository.getPtypefinishings().then(x=>x).then(x=>x);
        if(status==200)
          { 
    ...
    const columns = [
      ...
      {
        name: 'Image',
        cell: (row) => <img src={`${urlImg}system/public/dist/img/ptypefinishing/${row.image}`} style={{width:50,height:50,borderRadius:50,}}/>
      }
     , {
       name: 'Product type',
       selector: 'ptypex',
      ...
      ,
      {
        ...
        cell: (row) => <Grid item>
          <ButtonGroup variant="contained" size="small" aria-label="Small contained button group">
            <Button onClick={() => { history.push('/admin/edit/Updatedinishing',{record:row}) }}>Update</Button>
