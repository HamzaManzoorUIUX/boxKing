...
export default () => {
  ...
  const updateRecord=async (id,statusx)=>{
    ...
    const columns = [
  
        {
          ...
          cell: (row) => {
            if(row.order_items.length>0)
            {
                ...
                <img style={{width:50,height:50,marginLeft:22}} src={urlImg+"dist/img/upload/"+row.order_items[0].product.image}/>
            ...
            {
                return <p style={{marginLeft:22}}>No Products</p>
            }
        ...
        {
            name: 'Total Products',
        cell: (row) => <p style={{marginLeft:22}}>{row.order_items.length}</p>
          }
          ,
        {
            name: 'Order Status',
            cell: (row) => <p style={{color:'#F5AA75',display:'flex',marginLeft:22}}><div style={{width:10,height:10,borderRadius:10,marginTop:3,marginRight:10,backgroundColor:'#F5AA75'}}></div>{row.orderStatus}</p>
          }
          ,
        {
            name: 'Total Amount',
      ...
      {
        name: 'Action',
        cell: (row) =><div style={{marginLeft:22}} className="tabDD">
        <Dropdown>
