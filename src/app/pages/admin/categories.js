...
export default () => {
  ...
  const deleteRecord=async (id)=>{
    setShowModal(false);
    const {data,status} =await repository.deleteCategory(id).then(x=>x).then(x=>x);
    try {
      if (status != 200) {
        setOpen(true)
        ...
        setOpen(true)
        setMessage("Success! Category deleted sucessfully");
  ...
  const columns = [
   ...
   {
     name: 'Image',
     cell: (row) => <img src={`${urlImg}system/public/dist/img/categories/${row.image}`} style={{width:50,height:50,borderRadius:50,}}/>
   }
  , {
    name: 'Description',
 ...
 ,
    {
      ...
      cell: (row) => <Grid item>
        <ButtonGroup variant="contained" size="small" aria-label="Small contained button group">
          <Button onClick={() => { history.push('/admin/edit/category',{record:row}) }}>Update</Button>
