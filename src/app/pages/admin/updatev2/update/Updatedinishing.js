...
export default function TextFields(props) {
  ...
  const postPType=async (datapost)=>{
    ...
    setloader(true);
    const {data,status}= await repository.updatePtypefinishing(datapost).then(x=>x).then(x=>x)
     try{
      if(status!=200)
      {
        setOpen(true)
...
<Notice icon="flaticon-warning font-primary">
      ...
      <CardHeader title="Product Type Finishing Options
"/>
      <CardBody>
      <Formik
       initialValues={{
      ...
      >
        {categories.map(option => (
          <MenuItem key={option.id} value={option.id}>
           ...
           <ReactQuill placeholder="Description" theme="snow" value={values.shortdescription} onChange={(e)=>{setFieldValue("shortdescription",e);console.log("sd",values.description)}}/>
