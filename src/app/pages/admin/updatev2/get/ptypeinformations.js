...
export default () => {
  ...
  const deleteRecord=async (id)=>{
    setShowModal(false);
    const {data,status} =await repository.deleteptypeinformation(id).then(x=>x).then(x=>x);
    try {
      if (status != 200) {
        setOpen(true)
        ...
        const {data,status}  =await repository.getptypeinformations().then(x=>x).then(x=>x);
        if(status==200)
          { 
      ...
      ,
      {
        ...
        cell: (row) => <Grid item>
          <ButtonGroup variant="contained" size="small" aria-label="Small contained button group">
            <Button onClick={() => { history.push('/admin/edit/UpdatePtypeinformations',{record:row}) }}>Update</Button>
