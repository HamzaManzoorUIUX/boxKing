...
export default function TextFields() {
  ...
  const postCat=async (datapost)=>{
    setloader(true);
    const {data,status}= await repository.postfinishingeffect(datapost).then(x=>x).then(x=>x)
     try{
      if(status!=200)
      {
        setOpen(true)
      ...
      {
        setOpen(true)
        setMessage("Success! Finishing Effect added sucessfully")
      }
...
<Notice icon="flaticon-warning font-primary">
      ...
      <CardHeader title="Finshing Effect"/>
      <CardBody>
      <Formik
       initialValues={{
        name: '',
