...
export default () => {
  ...
  const deleteRecord=async (id)=>{
    setShowModal(false);
    const {data,status} =await repository.deleteuser(id).then(x=>x).then(x=>x);
    try {
      if (status != 200) {
        setOpen(true)
        ...
        setOpen(true)
        setMessage("Success! User deactivated sucessfully");
        const {data,status}  =await repository.getUsers().then(x=>x).then(x=>x);
        if(status==200)
          { 
            setDatax(data);
  ...
  ,
      {
        name: 'Role',
        cell: (row)=>(row.role==null?"User":"Admin")
      },
      {
        name: 'Action',
        cell: (row) => <Grid item>
