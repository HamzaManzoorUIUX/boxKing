...
const history=useHistory();
  const deleteRecord=async (id)=>{
    setShowModal(false);
    const {data,status} =await repository.deletefinishingeffect(id).then(x=>x).then(x=>x);
    try {
      if (status != 200) {
        setOpen(true)
  ...
  const columns = [
    ...
    ,
    {
      ...
      cell: (row) => <Grid item>
        ...
        <Button onClick={() => { history.push('/admin/edit/finishingeffect',{record:row}) }}>Update</Button>
