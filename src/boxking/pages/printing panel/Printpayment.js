  ...
  const updateRecord=async (id,statusx)=>{
    ...
    try {
      if (status != 200) {
        ...
        const { data, status } = await repository.paymentsbyCompany(user.id).then(x => x).then(x => x);
        if (status == 200) {
          setDatax(data);
    ...
    const columns = [
  
        {
          ...
          cell: (row) => {
            ...
            return  <p style={{marginLeft: 22}}>{row.source}</p>
        ...
        ,
        {
            name: 'Description',
            width	:'300px',
        cell: (row) => <p style={{marginLeft: 22}}>{row.description}</p>
          }
          ,
        {
            name: 'Amount',
            cell: (row) =><p style={{marginLeft: 22}}>{row.amount}</p>
          }
          ,
         {
            name: 'User Email',
            cell: (row) =><p style={{marginLeft: 22}}>{row.user.email}</p>
          }
          ,
          {
              name: 'Date',
              cell: (row) =><p style={{marginLeft: 22}}>{row.created_at.substring(0, 10)}</p>
            }
