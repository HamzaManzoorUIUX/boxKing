...
export default function TextFields(props) {
  ...
  const [model, setModel] = useState(record.modelPath);
  const [uvImage, setuvImage] = useState(record.uvImage);
  ...
  const postCat = async (datapost) => {
    setloader(true);
    const { data, status } = await repository.updateProduct(datapost).then(x => x).then(x => x);
    try {
      if (status != 200) {
        setOpen(true)
                        ...
                        <input
                          ...
                          onChange={e => {
                            ...
                            reader.onloadend = function () {
                              ...
                              setFieldValue("modelChange", true)
                              setModel(file.name)
