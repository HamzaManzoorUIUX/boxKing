...
export default function TextFields(props) {
  ...
  const postCat=async (datapost)=>{
    setloader(true);
    const {data,status}= await repository.updatefinishing(datapost).then(x=>x).then(x=>x)
     try{
      if(status!=200)
      {
        setOpen(true)
      ...
      {
        setOpen(true)
        setMessage("Success! Finishing Updated sucessfully")
      }
