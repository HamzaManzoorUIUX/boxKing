...
export default function TextFields(props) {
  ...
  const postCat=async (datapost)=>{
    setloader(true);
    const {data,status}= await repository.updatefinishingeffect(datapost).then(x=>x).then(x=>x)
     try{
      if(status!=200)
      {
        setOpen(true)
      ...
      {
        setOpen(true)
        setMessage("Success! Finishing Effect Updated sucessfully")
      }
...
<Notice icon="flaticon-warning font-primary">
      ...
      <CardHeader title="Finishing Effect"/>
      <CardBody>
      <Formik
       initialValues={{
        name: record.name,
