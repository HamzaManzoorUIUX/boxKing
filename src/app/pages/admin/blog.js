...
export default function TextFields() {
  ...
  const postPType=async (datapost)=>{
    setloader(true);
    const {data,status}= await repository.postblog(datapost).then(x=>x).then(x=>x)
     try{
      if(status!=200)
      {
        setOpen(true)
      ...
      {
        setOpen(true)
        setMessage("Success! Blog added sucessfully")
      }
  ...
  const getCat=async ()=>{
    ...
    const {data,status}= await repository.blogs().then(response => response).then(x=>x);
    if(status!=200)
    {
      setCategories([]);
     }
