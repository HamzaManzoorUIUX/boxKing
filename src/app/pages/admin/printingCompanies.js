...
export default () => {
  ...
  const deleteRecord=async (id)=>{
    setShowModal(false);
    const {data,status} =await repository.deletePrintingCompanies(id).then(x=>x).then(x=>x);
    try {
      if (status != 200) {
        setOpen(true)
        ...
        setOpen(true)
        setMessage("Success! Printing Company deleted sucessfully");
    ...
    const columns = [
      ...
      {
        name: 'Premium Shipping Amount',
        selector: 'premiumshipping',
        sortable: true,
      }
      ,
      ...
      
      {
        ...
        cell: (row) => <Grid item>
          <ButtonGroup variant="contained" size="small" aria-label="Small contained button group">
            <Button onClick={() => { history.push('/admin/edit/printingCompany',{record:row}) }}>Update</Button>
