...
export default function TextFields(props) {
  ...
  const postPType=async (datapost)=>{
    setloader(true);
    const {data,status}= await repository.updateProductType(datapost).then(x=>x).then(x=>x)
     try{
      if(status!=200)
      {
        setOpen(true)
      ...
      {
        setOpen(true)
        setMessage("Success! Product Type updated sucessfully")
      }
      ...
      >
        {categories.map(option => (
          <MenuItem key={option.id} value={option.id}>
           ...
           <ReactQuill placeholder="Description" theme="snow" value={values.description} onChange={(e)=>{setFieldValue("description",e);console.log("sd",values.description)}}/>
